...
with tabs[0]:
    start_card("Digital Signature (RSA)")
    ...
    st.success("Digital signatures provide integrity and authenticity.")
    end_card()
...
with tabs[1]:
    ...
    st.markdown("#### Why it is important")
    st.success("Hashes help detect changes and verify data integrity.")
    end_card()

with tabs[2]:
    start_card("Password Strength Analyzer")
    st.info("**What is happening:** Password is evaluated against simple strength rules.")
    ...
    if password:
        ...
        if tips:
            ...
            for tip in tips:
                st.write(f"- {tip}")
        add_log("password_analysis", f"Password analyzed: {label}", actor=st.session_state.current_user or "guest")
