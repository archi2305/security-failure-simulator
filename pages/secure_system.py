...
with tabs[0]:
    start_card("Registration with bcrypt + salt")
    ...
    st.markdown("#### Why it is important")
    st.success("If database leaks, plain passwords are not exposed.")
    end_card()

with tabs[1]:
    start_card("Login with bcrypt verification")
...
with tabs[2]:
    ...
    st.info("**What is happening:** System allows or denies actions based on user role.")
    current = st.session_state.current_user
    if not current:
        st.warning("Login first to test role behavior.")
    else:
        role = st.session_state.users_db[current]["role"]
        c1, c2, c3 = st.columns(3)
        c1.write("Input")
        c1.code(f"user={current}\nrole={role}")
        c2.write("Processing")
        ...
        if role == "admin":
            ...
            if st.button("Admin action: Audit users"):
                add_log("admin_action", "User audit executed", actor=current)
                st.success("Admin action completed.")
        else:
            c3.error("Admin actions blocked")
            st.warning("You are logged in as normal user.")
    st.success("RBAC limits privilege misuse and supports least privilege.")
    end_card()

with tabs[3]:
    start_card("AES-based File/Text Encryption (Fernet)")
